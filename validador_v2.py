...
def parse_apontamentos(path):
    ...
    for idx in range(header_row + 2, len(raw)):
        row = raw.iloc[idx]
        # Ignora linhas de total/resumo (col0 contém texto como TOTAL, SUBTOTAL, etc)
        col0 = str(row.iloc[0]).strip().upper()
        if col0 in ('TOTAL', 'SUBTOTAL', 'TOTAIS', 'RESUMO', 'SOMA') or 'TOTAL' in col0:
            continue
        mat_raw = row.iloc[1]
...
def parse_coop_excel(path):
    ...
    col_cod = col_227 = col_203_tit = col_203_dep = col_nome = None
    for i, h in enumerate(header):
        h = str(h).strip()
        if h in ('Cód.','Cod.','Código'): col_cod = i
        elif '227' in h: col_227 = i
        elif '203' in h and ('Dep' in h or 'dep' in h): col_203_dep = i
        elif '203' in h: col_203_tit = i
        elif 'nome' in h.lower() or 'func' in h.lower() or 'Colabor' in h: col_nome = i
    if col_cod is None: col_cod = 1
    if col_227 is None: col_227 = 3
    if col_203_tit is None: col_203_tit = 6
    if col_203_dep is None: col_203_dep = 7
    ...
    for idx in range(header_row + 1, len(raw)):
        ...
        if mat <= 0: continue
        nome = str(row.iloc[col_nome]).strip() if col_nome and not pd.isna(row.iloc[col_nome]) else ''
        v227     = _to_float(row.iloc[col_227])
        v203_tit = _to_float(row.iloc[col_203_tit]) if col_203_tit is not None and col_203_tit < len(row) else 0.0
        v203_dep = _to_float(row.iloc[col_203_dep]) if col_203_dep is not None and col_203_dep < len(row) else 0.0
        v203_total = v203_tit + v203_dep

        if mat in result:
            result[mat]['227']     += v227
            result[mat]['203']     += v203_total
            result[mat]['203_tit'] += v203_tit
            result[mat]['203_dep'] += v203_dep
        else:
            result[mat] = {'227': v227, '203': v203_total,
                           '203_tit': v203_tit, '203_dep': v203_dep, 'nome': nome}
        if v227 > 0:
        ...
        if v203_tit > 0:
            raw_rows.append({'origem':'COOPARTICIPACAO','cod_emp':mat,'nome_input':nome,'rubrica':'203',
                             'descricao_input':'Fator Moderador — Titular','valor_esperado':v203_tit,'status_input':'VALOR'})
        if v203_dep > 0:
            raw_rows.append({'origem':'COOPARTICIPACAO','cod_emp':mat,'nome_input':nome,'rubrica':'203',
                             'descricao_input':'Fator Moderador — Dependente','valor_esperado':v203_dep,'status_input':'VALOR'})
    return result, raw_rows
...
def analisar(funcs, apo_data, apo_raw, emp_list, coop_data, coop_raw):
            ...
            if not excel_was_filled:
                if vf > 0:
                    ...
                    pontos_atencao.append({
                        ...
                        'rubrica': cod, 'descricao_input': DESCRICOES.get(cod, desc),
                        'valor_esperado': None, 'referencia_esperada': None,
                        'status_input': None,
                        'obs_input': f'Rubrica {cod} ({DESCRICOES.get(cod, desc)}) presente na folha (R${vf:.2f}) sem input numerico correspondente nas planilhas auxiliares.',
                        'valor_folha': vf, 'referencia_folha': raw_entry.get('val'),
                        'diferenca': None, 'status': 'ATENÇÃO',
                        'critica': f'Rubrica {cod} lançada na folha R${vf:.2f} sem registro no Excel de apontamentos.',
            ...
            if not excel_filled:
                ...
                if vf > 0:
                    pontos_atencao.append({
                        ...
                        'valor_esperado':None,'referencia_esperada':None,
                        'status_input':None,
                        'obs_input':f'Rubrica {cod} ({desc}) presente na folha (R${vf:.2f}) sem horas informadas no Excel de apontamentos.',
                        'valor_folha':vf,'referencia_folha':None,
                        'diferenca':None,'status':'ATENÇÃO',
                        'critica':f'Rubrica {cod} ({desc}) lançada na folha R${vf:.2f} sem registro no Excel.',
    ...
    # ── Coparticipação: compara 203 (titular + dependente) e 227 ────
    # A conferência principal é a rubrica 203 (Fator Moderador)
    # 227 é informativa e não gera divergência, apenas ponto de atenção se ausente

    all_mats_coop = set(coop_data.keys()) | {
        m for m, f in funcs.items() if f.rubricas.get('203', 0) > 0
    }

    for mat in sorted(all_mats_coop):
        func = funcs.get(mat)
        nome_input = coop_data.get(mat, {}).get('nome', '')
        nome = func.nome if func else nome_input

        # ── Rubrica 203: titular ──────────────────────────────────
        v203_tit_excel = coop_data.get(mat, {}).get('203_tit', 0.0)
        v203_dep_excel = coop_data.get(mat, {}).get('203_dep', 0.0)
        v203_excel     = coop_data.get(mat, {}).get('203', 0.0)  # total
        v203_folha     = func.rubricas.get('203', 0.0) if func else 0.0

        if v203_excel > 0 or v203_folha > 0:
            diff = round(v203_excel - v203_folha, 2)
            desc_excel = []
            if v203_tit_excel > 0: desc_excel.append(f'Titular: R${v203_tit_excel:.2f}')
            if v203_dep_excel > 0: desc_excel.append(f'Dependente: R${v203_dep_excel:.2f}')
            desc_str = ' | '.join(desc_excel) if desc_excel else 'Não informado'

            row = {
                'origem': 'COOPARTICIPACAO', 'cod_emp': mat, 'nome_input': nome,
                'rubrica': '203',
                'descricao_input': f'Fator Moderador Unimed ({desc_str})',
                'valor_esperado': v203_excel if v203_excel > 0 else None,
                'referencia_esperada': None,
                'status_input': 'VALOR' if v203_excel > 0 else None,
                'obs_input': None,
                'valor_folha': v203_folha if v203_folha > 0 else None,
                'referencia_folha': None,
                'diferenca': diff,
                'status': 'OK' if abs(diff) <= TOLERANCE else 'DIVERGÊNCIA',
                'critica': 'Coerente.' if abs(diff) <= TOLERANCE else
                           (f'Rubrica 203 no Excel (R${v203_excel:.2f}) não encontrada na folha.' if v203_folha == 0 else
                            f'Valor 203 Excel R${v203_excel:.2f} difere folha R${v203_folha:.2f}.')
            }
            if abs(diff) > TOLERANCE: divergencias.append(dict(row))
            comparativo.append(row)

        # ── Rubrica 227: informativa — apenas ponto de atenção ────
        v227_excel = coop_data.get(mat, {}).get('227', 0.0)
        v227_folha = func.rubricas.get('227', 0.0) if func else 0.0
        if v227_excel > 0 or v227_folha > 0:
            diff227 = round(v227_excel - v227_folha, 2)
            row227 = {
                'origem': 'COOPARTICIPACAO', 'cod_emp': mat, 'nome_input': nome,
                'rubrica': '227',
                'descricao_input': 'Plano de Saúde Informativa',
                'valor_esperado': v227_excel if v227_excel > 0 else None,
                'referencia_esperada': None,
                'status_input': 'VALOR' if v227_excel > 0 else None,
                'obs_input': None,
                'valor_folha': v227_folha if v227_folha > 0 else None,
                'referencia_folha': None,
                'diferenca': diff227,
                'status': 'OK' if abs(diff227) <= TOLERANCE else 'DIVERGÊNCIA',
                'critica': 'Coerente.' if abs(diff227) <= TOLERANCE else
                           f'Rubrica 227 Excel R${v227_excel:.2f} difere folha R${v227_folha:.2f}.'
            }
            if abs(diff227) > TOLERANCE: divergencias.append(dict(row227))
            comparativo.append(row227)

    # ── Empréstimos: cruzamento por contrato individual ─────────────
    # Monta índice da folha por contrato
    folha_contratos = {}
    for func in funcs.values():
        for loan in func.emprestimos:
            folha_contratos[loan['contrato'].strip()] = {
                'valor': loan['valor'], 'rubrica': loan['rubrica'],
                'matricula': func.matricula, 'nome': func.nome
            }

    excel_contratos_vistos = set()
    for rec in emp_list:
        contrato = str(rec['contrato']).strip()
        excel_contratos_vistos.add(contrato)
        vx = rec['valorParcela']
        nome_excel = rec['nome']

        if contrato in folha_contratos:
            info = folha_contratos[contrato]
            vf = info['valor']
            diff = round(vx - vf, 2)
            status = 'OK' if abs(diff) <= TOLERANCE else 'DIVERGÊNCIA'
            critica = 'Coerente.' if status == 'OK' else f'Parcela Excel R${vx:.2f} difere da folha R${vf:.2f}.'
            emp_resultado.append({
                'nome_norm': info['nome'],
                'nome_input': nome_excel,
                'contrato': contrato,
                'rubrica': info['rubrica'],
                'valor_input': vx,
                'contratos_input': contrato,
                'nome_folha': info['nome'],
                'valor_folha': vf,
                'rubricas': info['rubrica'],
                'diferenca': diff if abs(diff) > TOLERANCE else 0,
                'status': status,
                'critica': critica
            })
        else:
            # Contrato no Excel mas não descontado na folha
            emp_resultado.append({
                'nome_norm': nome_excel,
                'nome_input': nome_excel,
                'contrato': contrato,
                'rubrica': '—',
                'valor_input': vx,
                ...
                'diferenca': vx,
                'status': 'DIVERGÊNCIA',
                'critica': f'Contrato {contrato} no Excel mas NÃO descontado na folha.'
            })

    # Contratos na folha ausentes no Excel
    for contrato, info in folha_contratos.items():
        if contrato not in excel_contratos_vistos:
            emp_resultado.append({
                'nome_norm': info['nome'],
                'nome_input': None,
                'contrato': contrato,
                'rubrica': info['rubrica'],
                'valor_input': None,
                'contratos_input': None,
                'nome_folha': info['nome'],
                'valor_folha': info['valor'],
                'rubricas': info['rubrica'],
                'diferenca': -info['valor'],
                'status': 'DIVERGÊNCIA',
                'critica': f'Contrato {contrato} descontado na folha mas AUSENTE no Excel.'
            })
...
def gerar_relatorio(funcs, apo_data, apo_raw, emp_list, coop_data, coop_raw,
    ...
    _set_col_widths(ws5, [35,35,20,14,14,14,12,14,60])
    _title_row(ws5, 1, 'Empréstimos — Comparativo por contrato individual', 9, bg=C_AZUL)
    hemp = ['nome_input','nome_folha','contrato','valor_input','valor_folha','diferenca','rubrica','status','critica']
